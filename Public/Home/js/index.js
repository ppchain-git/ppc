...
function msg_alert(message,url){
  ...
  if(url){
    layer.msg(message,{time:1000 , skin: 'demo-class'},function(){
          window.location.href=url;
        });
  }else{
    layer.msg(message,{time:1500 , skin: 'demo-class'});
  }
...
function change_code(){
  var verifyURL="/Home/Login/verify";
...
function en_adduser(){
    ...
    if(username=='' || username==null){
        msg_alert('Name cannot be empty');
        return false;
      }
	  
	  if(mobile=='' || mobile==null){
        msg_alert('The phone number can not be empty.');
        return false;
      }
      if(!mobile_rex.test(mobile)){
        msg_alert('Wrong phone number');
        return false;
      }
	  
	  if(login_pwd=='' || login_pwd==null){
        msg_alert('The login password can not be empty.');
        return false;
      }
	  if(!reg.test(login_pwd)){
        msg_alert('The login password can only be 6-32 digit plus password.');
        return false;
      }
	  

      if(login_pwd!=relogin_pwd){
          msg_alert("Two logon passwords do not match.");
          return false;
      }
	  
    if(!regjyi.test(safe_pwd)){
        msg_alert('The transaction password can only be 6 digits.');
        return false;
    }
    if(code=='' || code==null){
        msg_alert('The verification code can not be empty.');
        return false;
    }
...
function SetPwd(){
	...
	if(!reg.test(passwordmin)){
		msg_alert('请输入6-32位数字加字母的确认密码！');
		return false;
	}
